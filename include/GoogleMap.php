...
class GoogleMapAPI {
     ...
     */
    var $directions_unit_system = 'METRIC';
     ...
     */
    function GoogleMapAPI($map_id = 'GBMAP', $app_id = 'GBMAPApp') {
        $this->map_id = $map_id;
	...
	*/
    ...
    function getMapJS() {
		...
		if($this->street_view_dom_id!=""){
			$_script .= " var panorama".$this->street_view_dom_id."$_key = '';\n";
			if(!empty($this->_markers)){
				$_script .= " var panorama".$this->street_view_dom_id."markers$_key = [];\n";
			}
		}
		
		if(!empty($this->_markers)){
			$_script .= " var markers$_key  = [];";
			if($this->sidebar) {        
				$_script .= " var sidebar_html$_key  = '';\n var marker_html$_key  = [];\n";
			}
		}
		if($this->marker_clusterer){
			$_script .= " var markerClusterer$_key = null;\n";
		}        
        if($this->directions) {        
            $_script .= " var to_htmls$_key  = [];\n var from_htmls$_key  = [];\n";
        }
		if(!empty($this->_directions)){
			$_script .= " var directions$_key = [];\n";
		}
		//Polylines
		if(!empty($this->_polylines)){
			$_script .= " var polylines$_key = [];\n var polylineCoords$_key = [];\n";
			if(!empty($this->_elevation_polylines)){
				$_script .= " var elevationPolylines$_key = []\n;";
			}
		}
        //Polygons
		if(!empty($this->_polygons)){
			$_script .= " var polygon$_key = [];\n var polygonCoords$_key = [];\n";
		}
		//Elevation stuff
		if(!empty($this->_elevation_polylines)||(!empty($this->_directions)&&$this->elevation_directions)){
			$_script .= " var elevationCharts$_key = [];\n";
		}
		//Overlays
		if(!empty($this->_overlays)){
			$_script .= " var overlays$_key = [];\n";
		}
        //KML Overlays
        if(!empty($this->_kml_overlays)){
            $_script .= " var kml_overlays$_key = [];\n";
        }
        //New Icons
	 ...
	 */
	 ...
	 function getUtilityFunctions(){
         ...
         $_script .= 'function hide(id) { polylines'.$this->map_id.'[id].setVisible(false); }'."\n";
         $_script .= 'function show(id) { polylines'.$this->map_id.'[id].setVisible(true); }'."\n";
         
         $_script .= 'function toggle(id) { ';
         $_script .= 'if (polylines'.$this->map_id.'[id].visible == true ) polylines'.$this->map_id.'[id].setVisible(false);';
         $_script .= 'else polylines'.$this->map_id.'[id].setVisible(true);';
         $_script .= '}'."\n";
		 return $_script;
	 }
    ...
    function getPolylineJS() {
        ...
        foreach($this->_polylines as $polyline_id =>$_polyline) {
        	...
        	foreach($_polyline["coords"] as $_coords){
        		...
        		$_coords_output .= " new google.maps.LatLng(".$_coords["lat"].", ".$_coords["long"].") ";
        	}
        	$_output .= "
              ...
              polylines".$this->map_id."[$polyline_id].setVisible(true);
        	";
